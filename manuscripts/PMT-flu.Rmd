...
## Hypotheses
We hypothesise that intent to obtain a seasonal influenza vaccination, will have a significant positive relationships with percieved severity of and susceptibility to influenza, as well as percieved efficacy of the vaccine (response efficacy), and their confidence in their ability to obtain a vaccination (self-efficacy), and negative relationships with maladaptive response rewards and percieved response costs.  
...
pmt <- apa_print(pmt_flu)

pmt$table <- as.tbl(pmt$table) %>% 
  mutate(
...
apa_table(pmt$table, escape = FALSE, note = pmt$full_result$modelfit$r2, caption = "Linear regression of vaccination intention on PMT constructs.", align = "lccccc")
